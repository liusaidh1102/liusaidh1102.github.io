...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | liusaidh's Blog🍋")[0];
}
...
$.ajax({
  ...
  data: {
    key: 'LCUBZ-RF3W3-KBR3M-RZYYL-JZJIZ-F6BV7',
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  // 经纬度
  let dist = getDistance(113.94,35.28 , ipLoacation.result.location.lng, ipLoacation.result.location.lat);
        ...
        case "河南省":
          switch (ipLoacation.result.ad_info.city) {
            ...
            case "洛阳市":
              ...
              break;
            case "新乡市":
              posdesc = "新乡的风教会我一个道理：人生的方向，就像我的发型，该顺拐时就顺拐。";
              break;
            default:
              posdesc = "可否带我品尝河南烩面啦？";
              break;
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by liusaidh🍋 %c 你正在访问liusaidh🍋の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| liusaidh🍋") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('liusaidh🍋的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://coderliu.top/static/img/background/default_bg.png);
    --darkmode-bg:url(https://coderliu.top/static/img/background/darkmode_bg.png);
    --mobileday-bg: url(https://coderliu.top/static/img/background/mobileday_bg.webp);
    --mobilenight-bg: url(https://coderliu.top/static/img/background/mobilenight_bg.webp);
  }`;
}
// 切换背景主函数
function changeBg(s) {
